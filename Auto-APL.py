...
#
# Usage:
# python lcu-mm-auto-accept-auto-lock-champion.py "Leesin" "Kha\'Zix"
# 
...
while True:
    ...
    if phase == 'ReadyCheck':
        ...
        sleep(5)
        # c = request('get', '/lol-champ-select/v1/session')
        # for player in c.json()['myTeam']:
            # userIds.append(str(player['summonerId']))
            # userIdsStringify = ','.join(map(str, userIds))
            # check = request('get', '/lol-summoner/v2/summoner-names?ids=[%s]' % userIdsStringify)
            # if check.status_code == 200:
                # for summonerInfo in check.json():
                    # summonerNames.append(summonerInfo['displayName'])
                    # if range(len(summonerNames)) == 5:
                    # if len(summonerNames) == 5:
                        # break
        ...
        # for i in range(len(res)):
            # if str(res[i]) != 'ŁĘĘˆŠÏŃ XƯNG ßÁ':
                # webbrowser.open( 'https://lmssplus.com/profile/' + str(res[i]) )
